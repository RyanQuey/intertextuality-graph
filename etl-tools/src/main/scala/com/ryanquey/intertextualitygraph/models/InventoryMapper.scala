...
@Mapper trait InventoryMapper {
  @DaoFactory def bookDao (@DaoTable table : String) : BookDao;

  MapperBuilder[InventoryMapper] builder(session : CqlSession) {
    ...
    new InventoryMapperBuilder(session);
  }
}
