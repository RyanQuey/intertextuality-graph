 ...
 */
class VersesController @Inject()(cc: ControllerComponents) extends AbstractController(cc) {

  def findOne(bookName : String, chapterNumber : Int, verseNumber : Int) = Action { implicit request: Request[AnyContent] =>

    // retrieve one from the database
    val verse = VerseRecord.dao().findOne(bookName, chapterNumber, verseNumber)
    ...
    //
    val verseJson = mapper.writeValueAsString(verse)
